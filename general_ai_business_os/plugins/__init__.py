...
class PluginRegistry:
    ...
    def register(self, plugin_id: str, manifest: Mapping[str, Any]) -> None:
        if not isinstance(plugin_id, str) or not isinstance(manifest, Mapping) or set(manifest) != {"plugin_version", "core_api"} or not all(isinstance(value, str) for value in manifest.values()): raise ValueError("plugin_manifest_invalid")
